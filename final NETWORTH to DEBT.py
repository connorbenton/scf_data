...
def get_figure() -> go.Figure:
    ...
    for k in ["MRTHEL","RESDBT","INSTALL","CCBAL","OTHLOC","ODEBT"]:
        DASH[k] = "dash"
    ...
    req = ["networth","fin","nfin","debt","x14","x42001",
           "mrthel","resdbt","install","ccbal","othloc","odebt"]
    ...
    # =============================
    # NW-aligned curves with DEBT tertiary decomposition
    # =============================
    DEBT_SUBS = ["mrthel","resdbt","install","ccbal","othloc","odebt"]

    def nw_aligned_with_debt_tertiary(df, age_mask, centers_probs, edges_probs, smooth_window=5):
        base = df.copy()
        cols = ["networth","fin","nfin","debt","wgt"] + DEBT_SUBS
        for c in cols:
        ...
        sub_means  = {k: np.full(n_bins, np.nan) for k in DEBT_SUBS}
        ...
        for b in range(n_bins):
            ...
            if not np.isfinite(nwbar) or nwbar <= 0: continue
            nw_means[b]   = nwbar
        ...
        # Shares inside DEBT
        with np.errstate(divide="ignore", invalid="ignore"):
            s_sub = {k: sub_means[k] / debt_means for k in DEBT_SUBS}
        ...
        if smooth_window and smooth_window > 1:
            ...
            for k in DEBT_SUBS: s_sub[k] = smooth(s_sub[k])

        # Compose curves
        fin_curve  = r_fin  * nw_curve
        nfin_curve = r_nfin * nw_curve
        debt_curve = r_debt * nw_curve
        debt_sub_curves = {k: s_sub[k] * debt_curve for k in DEBT_SUBS}

        # Mask small
        cut = MASK_THRESH * nw_curve
        def mask(a):
            a[(~np.isfinite(a)) | (~np.isfinite(nw_curve)) | (nw_curve <= 0) | (a < cut)] = np.nan
            return a
        fin_curve, nfin_curve, debt_curve = map(mask, [fin_curve, nfin_curve, debt_curve])
        for k in DEBT_SUBS: debt_sub_curves[k] = mask(debt_sub_curves[k])

        # Rescale DEBT subs to sum to DEBT
        subsum = np.nansum([debt_sub_curves[k] for k in DEBT_SUBS], axis=0)
        with np.errstate(divide="ignore", invalid="ignore"):
            sub_scale = np.where(np.isfinite(subsum) & (subsum > 0) & np.isfinite(debt_curve),
                                 debt_curve / subsum, np.nan)
        for k in DEBT_SUBS:
            ok = np.isfinite(sub_scale)
            debt_sub_curves[k][ok] *= sub_scale[ok]

        # Enforce FIN + NFIN − DEBT = NW
        ...
        if np.any(ok):
            ...
            for k in DEBT_SUBS: debt_sub_curves[k][ok] *= tri_scale[ok]

        return {"nw": nw_curve, "fin": fin_curve, "nfin": nfin_curve, "debt": debt_curve,
                "debt_subs": debt_sub_curves, "probs": centers_probs}
    ...
    for lbl in age_labels:
        res = nw_aligned_with_debt_tertiary(
        ...
        if res is None:
            ...
            curves_by_age[lbl] = {"px": px, "nw": empty, "fin": empty, "nfin": empty, "debt": empty,
                                  "debt_subs": {k: empty.copy() for k in DEBT_SUBS}}
            ...
            pools = [res["nw"], res["fin"], res["nfin"], res["debt"]] + [res["debt_subs"][k] for k in DEBT_SUBS]
    ...
    def build_tooltip(label):
        nw, fin, nfin, debt = [round100(curves_by_age[label][k]) for k in ["nw","fin","nfin","debt"]]
        subs = {k.upper(): round100(curves_by_age[label]["debt_subs"][k]) for k in DEBT_SUBS}
        texts=[]
        for i in range(len(curves_by_age[label]["px"])):
            lines=[]
    ...
    for key in ["NW","FIN","NFIN","DEBT"]:
        fig.add_trace(go.Scatter(
            ...
            name=NAME_MAP[key], hoverinfo="skip",
        ))

    # DEBT subcomponents (legendonly)
    for k in DEBT_SUBS:
        lab = k.upper()
        fig.add_trace(go.Scatter(
            x=px0, y=curves_by_age[default_age]["debt_subs"][k],
            mode="lines",
    ...
    fig.update_layout(
        ...
        showlegend=True,
        legend=dict(
            title="Toggle lines (click):<br><span style='font-weight:400'>(Debt subcomponents start hidden)</span>",
            x=0.02, y=0.98,
