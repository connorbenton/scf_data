...
def get_figure() -> go.Figure:
    ...
    SMOOTH_WIN = 35    # smoothing window for SHARES only (not NFIN)

    MASK_THRESH = 0.02   # per-bin masking threshold (2% of NFIN)
    TOL         = 0.02   # re-scale visible subs to be within ±2% of NFIN (we scale to exact)

    NAME_MAP = {
        "NFIN":   "Nonfinancial assets",
        "VEHIC":  "Vehicle equity",
        "HOUSES": "Primary residence equity",
        "ORESRE": "Other residential real estate equity",
        "NNRESRE":"Nonresidential real estate equity",
        "BUS":    "Business interests",
        "OTHNFIN":"Miscellaneous assets (gold, art, etc)",
    }
    ...
    master = "nfin"
    others = ["vehic", "houses", "oresre", "nnresre", "bus", "othnfin"]
    ...
    # =============================
    # NFIN-aligned curves (true NFIN percentiles, smoothed shares)
    # =============================
    def master_aligned_curves_binned(
        ...
        """
        - Master curve (NFIN) = true weighted percentile at centers_probs
        ...
        # NFIN percentile curve at requested centers
    ...
    def refine_and_mask_for_age(label, mask_thresh=MASK_THRESH, tol=TOL):
        f   = np.asarray(curves_by_age[label]["master"], float)  # master = NFIN curve
        px  = curves_by_age[label]["px"]
        comps = {c: np.asarray(curves_by_age[label][c], float).copy() for c in others}

        # 1) scale all comps to match NFIN per bin
        ...
        # 3) rescale visible to be within tol of NFIN (exact if possible)
        for i, fv in enumerate(f):
    ...
    DASH = {
        "NFIN":"solid",
        "VEHIC":"dash","HOUSES":"dash","ORESRE":"dash","NNRESRE":"dash",
        "BUS":"dash","OTHNFIN":"dash",
    }
    ...
    def build_sorted_tooltip_for_age(label, include_master=True):
        master_vals  = round100(refined_by_age[label]["nfin"])
        ...
        for i in range(n):
            ...
            if include_master and np.isfinite(master_vals[i]):
                lines.append(f"{swatch(COLOR['NFIN'],DASH['NFIN'])}<b>{NAME_MAP['NFIN']}</b>: {master_vals[i]:,.0f}")
    ...
    fig.add_trace(
        go.Scatter(
            x=px_default, y=refined_by_age[default_age]["nfin"],
    ...
    # NFIN line (visible; no own hover)
    fig.add_trace(
        go.Scatter(
            x=px_default, y=refined_by_age[default_age]["nfin"],
            mode="lines",
            line=dict(width=3, shape="spline", color=COLOR["NFIN"]),
            name=NAME_MAP["NFIN"],
            hoverinfo="skip",
        )
    )
    ...
    for lbl in age_labels:
        xs = [refined_by_age[lbl]["px"]] * (1 + 1 + n_sec)  # helper + NFIN + components
        ys = [refined_by_age[lbl]["nfin"],
            refined_by_age[lbl]["nfin"]] + [refined_by_age[lbl][v] for v in others]
        ...
        age_dropdown_buttons.append(
            dict(
                ...
                args=[
                    ...
                    # {"title": f"Weighted Percentile Curves — NFIN + Subcategories<br><sup>Age filter: {lbl}</sup>",
                    {"title": None,
    ...
    #     text=f"X-grid step = {STEP}. Tooltip lists NFIN, then subcategories sorted with matching swatches. Subcategories <2% of NFIN are omitted per bin.",
